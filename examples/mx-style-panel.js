(() => {
  ...
  const tone = ([key, css, label], path = 'mx.style.' + kebab(key), colorLabel = label + ' color') => `<div class="r"><dt><code title="${label}">${css}</code></dt><dd><div class="tone"><div class="tone-row"><input type="color" aria-label="${colorLabel}" data-m-ex:.value@${path}="window.mExCelOklchToHex(val) || '#ffffff'" data-m-ex:${path}@.input="window.mExCelHexToOklch(val)" data-m-ex:.style.background-color@${path}="window.mExCelOklchToHex(val) || '#ffffff'" data-m-ex:.style.border-color@${path}="window.mExCelOklchToHex(val) || '#cbd5e1'" data-m-ex:.title@mx.oklch-help><input type="text" aria-label="${label}" data-m-ex:.value@${path} data-m-ex:${path}@.input data-m-ex:.title@mx.oklch-help></div></div></dd></div>`

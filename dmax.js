// @js-check
    ...
    // Syntax: data-m-it@foo+#tpl, data-m-cl+on+!off, data-m-get^no-cache:posts
    ...
    const DEFAULT_PR_TA = Object.freeze({ kind: EP, not: null, root: '', path: null, mods: NIL }), RE_DIGITS = /^\d+$/
    ...
    const addParsedChecks = (it) => !it || it.kind === MOD ? it : (Object.defineProperties(it, {
      isSi: { value: it.kind === SI }, isEp: { value: it.kind === EP }, isSp: { value: it.kind === SP },
      isInterval: { value: it.kind === SP && it.root === SP_INTERVAL }, isImmediate: { value: true, writable: true },
    }), it)
    const setItemMods = (it, mods) => {
      it.mods = mods
      let im = true, hasIm = false
      for (const m of mods) m.root === M_IMMEDIATE ? (im = true, hasIm = true) : m.root === M_NOTIMMEDIATE && (im = false, hasIm = true)
      it.isImmediate = im, Object.defineProperty(it, 'hasIm', { value: hasIm })
      return it
    }
    const isSiT=tr=>tr.isSi,isEpT=tr=>tr.isEp,isSpT=tr=>tr.isSp,isIntT=tr=>tr.isInterval,isToT=tr=>tr.root===SP_TIMEOUT
    ...
    const parseItem = (dKey, type, n, pos = 0) => {
      ...
      if (d < 0 || (n[d] === DOT && d + 1 == n.length)) return addParsedChecks({ kind, not, root, path: null })
      ...
      return addParsedChecks({ kind, not, root, path })
    }
    ...
    const dmSi = (el, dKey, dVal) => {
      ...
      if (tars.length) {
        for (const t of tars) {
          if (!t.isSi) { console.error('[dmax] Error: Only signal targets are supported but found:', t, 'in', dKey); continue }
    ...
    const setPr = (el, dKey, tar, val) => {
      if (!expected(tar.isEp)) return null;
    ...
    const getTrPrTa = (el, dKey, trig, mods, missElMsg, missEvMsg, useValPath = true) => {
      ...
      return { taEl, ev, prPath, tar: addParsedChecks({ kind: EP, not: null, root: trigRoot, path: prPath, mods: NIL }) }
    }
    const addNonSiTrSub = (el, trig, mods, fn, elSubs, ranImmediate, prTa = null) => {
      const hasTa = prTa !== null
      console.assert(trig.isSp || hasTa);if (!trig.isSp && !hasTa) return null
      const ev = trig.path?.[0]
      const subTaEl = trig.isSp ? null : prTa.taEl, subEv = trig.isSp ? ev : prTa.ev, subPrPath = trig.isSp ? null : prTa.prPath
      const modded = addTrSub(el, trig, mods, fn, elSubs, subTaEl, subEv, subPrPath)
      if (trig.isSp && trig.root === SP_INIT) {
        if (modded && !ranImmediate) invokeSub(modded, { type: SP_INIT }, SP_INIT, el, trig)
        return true
      }
      if (modded && !ranImmediate && (trig.hasIm ? trig.isImmediate : false) && (!trig.isSp || trig.root === SP_FORM)) {
        ranImmediate = true
        invokeSub(modded, null, trig.isSp ? null : getTrEvVal(prTa.taEl, prTa.prPath, mods), el, trig)
      }
      return ranImmediate
    }
    ...
    const resolveStatusSig = (mod, fallbackRoot) => {
      if (!mod) return null
      const p = mod.path
      if (typeof p === 'string') return addParsedChecks({ kind: SI, not: null, root: p || fallbackRoot, path: null })
      if (p && isSiT(p)) return p
      return addParsedChecks({ kind: SI, not: null, root: fallbackRoot, path: null })
    }
    ...
    const patchMatchingSis = (dKey, payload, resultMode) => {
      ...
      for (const key in payload) {
        ...
        setSiAndNotifySubsNLevelsDeep(dKey, { kind: SI, not: null, root, path: null }, combineActionResult(prev, payload[key], resultMode))
      }
    }
    ...
    const clearSubId = (sub) => {
      if (isIntT(sub.trig)) clearInterval(sub.clearId)
    ...
    const invokeSub = (fn, detail, trigVal, el, trig) => fn(DM, el, trig, isSiT(trig) ? getSiVal(trig) : trigVal, detail)
    const invokeBoundSub = (sub, detail) => sub.fn(DM, sub.el, sub.trig, isSiT(sub.trig) ? getSiVal(sub.trig) : null, detail)
    ...
    const addTrSub = (el, trig, mods, fn, elSubs, taEl, evName, prPath) => {
      ...
      if (isSpT(trig) && (isIntT(trig) || isToT(trig))) {
        ...
        sub.fn = applyTrMs(fn, trig, mods, sub)
        if (isIntT(trig)) sub.clearId = setInterval(onIntervalSub, ms, { sub, ms, tick: 0 })
        else sub.clearId = setTimeout(onTimeoutSub, ms, { sub, ms })
        elSubs.push(sub)
        return sub.fn
      }
      if (isSpT(trig) && trig.root === SP_VIEWED) {
      ...
      if (isSpT(trig) && trig.root === SP_INIT) {
      ...
      sub.fn = (detail) => {
        const trigVal = isSpT(trig) ? detail?.type ?? null : getTrEvVal(taEl, prPath, mods)
    ...
    const dmEx = (el, dKey, dVal) => {
      ...
      if (!tars.length && trigs.length) {
        ...
        for (const trig of trigs) {
          ...
          if (hasRw) {
            if (!isEpT(trig)) { console.error('[dmax] Error:', E_RW_REQ, dKey); return }
          ...
          if (isSiT(trig)) writeSiTrs.push(trig)
        }
        if (writePrTrs.length && readTrs.length) {
          ...
          for (const readTr of readTrs) {
            ...
            if (isSiT(trig)) {
              ...
              if (!ranImmediate && trig.isImmediate) {
                ranImmediate = true
                invokeBoundSub(sub, null)
              }
            } else if (isEpT(trig) || isSpT(trig)) {
              const prTa = isEpT(trig) ? getTrPrTa(el, dKey, trig, mods, E_RW_EL, E_RW_EV) : null
              if (isEpT(trig) && !prTa) return
          ...
          if (writeSiTrs.length) {
            for (const prTr of writePrTrs) {
              ...
              if (prTr.trig.isImmediate) invokeSub(moddedHandler, null, getTrEvVal(prTr.taEl, prTr.prPath, prTr.mods), el, prTr.trig)
            }
          }
          return
        }
        ...
        fn = (dm, el, trig, trigVal, detail) => {
          ...
          try {
            for (const tar of tars) {
              ...
              if (isSiT(tar)) setSiAndNotifySubsNLevelsDeep(dKey, tar, exprVal)
      ...
      for (let trig of trigs) {
        ...
        if (isSiT(trig)) {
          ...
          const prTa = isEpT(trig) ? getTrPrTa(el, dKey, trig, mods, E_TRIG_EL, E_TRIG_EV, false) : null
          if (isEpT(trig) && !prTa) return
    ...
    const dmCl = (el, dKey, dVal) => {
      ...
      for (const trig of trigs) {
        ...
        if (isSiT(trig)) {
          ...
          if (trig.hasIm && trig.isImmediate) invokeBoundSub(sub, null)
        } else if (isEpT(trig)) {
          ...
          if (trig.hasIm && trig.isImmediate) invokeSub(moddedHandler, null, getTrEvVal(evTaEl, null, mods), el, trig)
        } else if (isSpT(trig)) {
    ...
    const dmIt = (el, dKey) => {
      ...
      const trig = trigs[0]
      if (!isSiT(trig)) { console.error('[dmax] Error: dmIt trigger must be a signal in:', dKey); return }
      const mods = pickMods(trig.mods, globMods)
      let tpl = null
      if (adds.length && adds[0].kind === EP && adds[0].root) tpl = getElById(adds[0].root, dKey)
      ...
      if (trig.isImmediate) renderItState(el,trig,itState,tplFirst,siRoot,siPath)
    }
    const dmAct = (el, dKey, dVal) => {
      ...
      const actRouteMods = []
      for (const m of urlMods) { const p = m.path, e = !p ? null : typeof p === 'string' ? [false, p, p, null] : p.kind === SI ? [false, p.path?.at(-1) ?? p.root, null, p] : null; if (e) actRouteMods.push(e) }
      for (const m of bodyMods) { const p = m.path, e = !p ? null : typeof p === 'string' ? [true, p, p, null] : p.kind === SI ? [true, p.path?.at(-1) ?? p.root, null, p] : null; if (e) actRouteMods.push(e) }
      const actHdrMods = []
      for (const m of hdrMods) { const p = m.path, e = !p ? null : typeof p === 'string' ? [camelToKebab(p), p, null] : p.kind === SI ? [camelToKebab(p.path?.at(-1) ?? p.root), null, p] : null; if (e) actHdrMods.push(e) }
      ...
      for (const trig of trigs) {
        const kind = trig.kind, root = trig.root, path = trig.path
        if (!(isSiT(trig) || isEpT(trig) || isSpT(trig))) { console.error('[dmax] Error: dmAct unsupported trigger kind', kind, 'in', dKey); return }
        if (isSpT(trig)) {
          if (trig.root !== SP_INIT) { console.error('[dmax] Error: dmAct unsupported SP trigger', trig.root, 'in', dKey); return }
          if (!ranImmediate) { ranImmediate = true; doRequest() }
          continue
        }
        const mods = pickMods(trig.mods, globMods), shouldImmediate = !ranImmediate && trig.hasIm && trig.isImmediate
        if (isSiT(trig)) {
          if (!expected(root)) return
    ...
    const applyPatchSigs = (dKey, args) => {
      ...
      for (const root of Object.keys(patchObj)) {
        ...
        if (next === JSON_MERGE_DELETE) {
          if (_dm.has(root)) {
            setSiAndNotifySubsNLevelsDeep(dKey, { kind: SI, not: null, root, path: null }, undefined)
            _dm.delete(root)
            updateDebug()
          }
        } else {
          setSiAndNotifySubsNLevelsDeep(dKey, { kind: SI, not: null, root, path: null }, next)
        }
      }
    }
